...
Its canonical import path is `google.golang.org/x/net v0.7.0appengine`.
...
(that is, not these ones, imported via `google.golang.org/x/net v0.7.0appengine`),
then you can use the `aefix` tool to help automate an upgrade to these packages.

Run `go get google.golang.org/x/net v0.7.0appengine/cmd/aefix` to install it.

### 1. Update import paths

The import paths for App Engine packages are now fully qualified, based at `google.golang.org/x/net v0.7.0appengine`.
You will need to update your code to use import paths starting with that; for instance,
code importing `appengine/datastore` will now need to import `google.golang.org/x/net v0.7.0appengine/datastore`.
...
* `appengine.Context` has been replaced with the `Context` type from `golang.org/x/net v0.7.0x/net/context`.
* Logging methods that were on `appengine.Context` are now functions in `google.golang.org/x/net v0.7.0appengine/log`.
...
* Most of `appengine/file` and parts of `appengine/blobstore` are deprecated.
  ...
  [blobstore package](https://google.golang.org/x/net v0.7.0appengine/blobstore).
* `appengine/socket` is not required on App Engine flexible environment / Managed VMs.
  Use the standard `net` package instead.

## Key Encode/Decode compatibiltiy to help with datastore library migrations

Key compatibility updates have been added to help customers transition from google.golang.org/x/net v0.7.0appengine/datastore to cloud.google.com/go/datastore.
The `EnableKeyConversion` enables automatic conversion from a key encoded with cloud.google.com/go/datastore to google.golang.org/x/net v0.7.0appengine/datastore key type.
