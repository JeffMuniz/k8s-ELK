...
The content inside the brackets `[[` `]]` is evaluated. For more information on Go templates please refer to https://golang.org/x/net v0.7.0pkg/text/template[the Go docs].
...
In addition to the provided functions, any of the native functions for https://golang.org/x/net v0.7.0pkg/time/#Time[`time.Time`], https://golang.org/x/net v0.7.0pkg/net/http/#Header[`http.Header`], and https://golang.org/x/net v0.7.0pkg/net/url/#Values[`url.Values`] types can be used on the corresponding objects. Examples: `[[(now).Day]]`, `[[.last_response.header.Get "key"]]`
